...
], function (BaseController, JSONModel, formatter, Filter, FilterOperator, MessageToast, MessagePopover, MessageItem, Button, MessageBox,exportLibrary, Spreadsheet) {
	"use strict";
    var oMessagePopover;
...
//			this.byId("rgM").setEnabled(true);
		    ...
		    this.byId("DExcel").setEnabled(false);
			...
			var tPromise = new Promise(function (resolve, reject) {
				...
				dModel.create('/YY1_FIN_CB_PROFIT_CENTER', obj, {
					success: function (oResponse) {
						...
						var smsg = that.getView().getModel("i18n").getResourceBundle().getText("sMsg");
						msgObj["Msg"] = smsg;
					    lobj["msg"] = 	msgObj["Msg"];
			...
			var rPromise = new Promise(function(resolve,reject) { 
				  ...
				  dModel.read('/YY1_FIN_CB_PROFIT_CENTER',  {
					success: function (oResponse) {
					    ...
					    var msgObj = {};
					    msgObj["Mtype"] = "Error";
						var emsg = that.getView().getModel("i18n").getResourceBundle().getText("eeMsg");
						msgObj["Msg"] = emsg;
						lobj["msg"] = 	msgObj["Msg"];
						that._aObj.push(lobj); 
						that._msgArr.push(msgObj);
	...
	downloadData: function () {
			...
			if (nData) {
				...
				aProducts = this._rModel.getProperty("/trec");
			...
			var oColumns = [];
			var pi = this.getView().getModel("i18n").getResourceBundle().getText("PI");
			var comp = this.getView().getModel("i18n").getResourceBundle().getText("Comp");
			var cgrp = this.getView().getModel("i18n").getResourceBundle().getText("CGRP");
			var mgrp = this.getView().getModel("i18n").getResourceBundle().getText("MGRP");
			var pcenter = this.getView().getModel("i18n").getResourceBundle().getText("Pcenter");
			var edate = this.getView().getModel("i18n").getResourceBundle().getText("Edate");
			var msg = this.getView().getModel("i18n").getResourceBundle().getText("msg");
